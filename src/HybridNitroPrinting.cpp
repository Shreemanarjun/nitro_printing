...
// Every capability is feature-detected at runtime and the best available
// path wins; what the sandbox truly cannot do fails honestly.
//
//   • Dialog printing (text/HTML/image/PDF): hidden-iframe + contentWindow
//     .print() — works on every browser.
//   • Raw / ESC-POS / ZPL, routed by PrintSettings.printerId:
//       "usb:VID:PID[:serial]" or empty → WebUSB bulk transfer (USB thermal /
//         label printers; grant devices via startPrinterDiscovery()).
//       "ws://host:port" / "wss://…"    → WebSocket→TCP relay (websockify).
//       "socket://host:port" / bare IP  → Direct Sockets TCPSocket (Isolated
//         Web Apps, Chrome 147+); otherwise fails with guidance.
//   • Printer enumeration / capabilities / status / PDF jobs: Web Printing
//     API (window.printing, Isolated Web Apps) merged with granted WebUSB
//     devices. The sync @NitroResult lookups serve from the cache the last
//     getAllPrinters() populated — call it first.
//   • printToFile: browser download.  renderPreview/getPageCount: pure-wasm
//     PDF passthrough / minimal text-to-PDF / page-tree count.
//
// Web constraint honored throughout: no threads — async work completes via JS
// callbacks into the exported nitro_printing_web_* functions below; the Dart
// side still observes asynchronous completion because port delivery is
// microtask-deferred. Posting protocol mirrors the desktop impl
// (linux/src/HybridNitroPrinting.cpp).
...
// renderPreview hands Dart a zero-copy pointer into native memory (Dart frees
// only the PreviewResult shell), so the bytes must stay alive; the previous
// preview is released when the next one is rendered.
std::vector<uint8_t>* g_lastPreview = nullptr;

// ── Minimal PDF utilities (pure wasm, no browser API) ────────────────────────

/// Naive page count: occurrences of "/Type /Page" that are not "/Pages".
/// ponytail: string scan, not a real xref parse — linearized/compressed
/// object streams undercount; swap in a real parser if that ever matters.
...
/// Page geometry for the generated-PDF paths, derived from PrintSettings
/// (paper size, orientation, margins). A4 portrait with default margins
/// yields exactly 60 lines/page — the contract getPageCount() documents.
struct PageGeom {
...
// Handles PDFs with plain "N G obj … endobj" objects and a classic page tree.
// Object streams (compressed xref) and encrypted files yield no objects — the
...
/// Extracts pages [from..to] (1-based, 0 = unbounded) into a standalone PDF
/// by rewriting the page tree — unused objects stay in the file, which keeps
/// every reference valid without dependency analysis. Empty result → caller
...
// ── JS flows ─────────────────────────────────────────────────────────────────
// Shared helpers are installed once on globalThis. Async flows end by calling
// back into the exported nitro_printing_web_* functions. JS→C++ strings are
// malloc'd UTF-8 the C++ side frees; structured data crosses as flat tables
// (fields \x1F, rows \x1E).
...
EM_JS(void, js_ensure_helpers, (), {
  ...
  // Self-contained (no emscripten runtime helpers — EM_JS bodies don't get
  // dependency auto-detection): UTF-8 encode into a malloc'd, NUL-terminated
  // buffer the C++ side frees.
  W.cstr = function(s) {
  ...
  // Packs the slices as [u32 count][u32 w,h,len]×count then the JPEG
  ...
  // One-shot query for IPP/raw printer services with the QU (unicast
  // response) bit, parsing PTR/SRV/A answers. Feature-detected; a browser
  // without bound-mode UDPSocket simply skips it.
  ...
  // ── Discovery ──────────────────────────────────────────────────────────
  // The device pickers need a user gesture — call from a button tap. Chain:
  // WebUSB picker; if unavailable/cancelled, the Web Serial picker; mDNS
  // (Isolated Web Apps, Direct Sockets UDP) always runs alongside — every
...
public:
    ...
    // The browser can always print via its dialog; USB / system printing
    // layers on where WebUSB / Web Printing are available.
    ...
    NitroCppBuffer getDefaultPrinter() override {
        // The web has no default-printer concept (not even in the Web
        // Printing API).
    ...
    /// PDF: a printerId naming a Web Printing system printer gets a REAL
    /// print job (silent, job-tracked, full IPP attributes); anything else
    ...
    /// Items route per type: plain text to a raw printerId goes out as
    /// ESC/POS, PDFs to a Web Printing printer become real jobs, everything
    /// else prints through the dialog — all sequentially.
    ...
    /// URLs (http(s)/data/blob) are fetched and printed by sniffed type —
    /// the web has no local filesystem, so plain paths return false.
    ...
    void printToFile(NitroCppBuffer document, const std::string& outputPath, NitroCppBuffer settings,
        ...
        // download named after outputPath's basename. Text and images render
        // to a real PDF first, matching the "virtual print to PDF" contract.
        PrintDocument doc = PrintDocument::fromNative(document);
        auto s = decodeSettings(settings);
    ...
    void printZpl(const std::string& zpl, NitroCppBuffer settings,
        ...
        // mirroring the desktop/Kotlin implementations.
